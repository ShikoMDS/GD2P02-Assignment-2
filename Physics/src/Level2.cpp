#include "Level2.h"
#include "TitleScreen.h"  // Assuming TitleScreen is the menu

Level2::Level2(SceneManager& manager) : sceneManager(manager), world(b2Vec2(0.0f, 9.8f)) {}

void Level2::init() {
    // Load the background texture
    if (!backgroundTexture.loadFromFile("resources/kenney physics assets/PNG/Backgrounds/colored_shroom.png")) {
        // Handle error
    }
    ...
    if (isPaused) {
        if (event.type == sf::Event::MouseButtonReleased && event.mouseButton.button == sf::Mouse::Left) {
            ...
            if (resumeButton.getGlobalBounds().contains(static_cast<float>(mousePos.x), static_cast<float>(mousePos.y))) {
                ...
                // Reset the level using setScene
                sceneManager.setScene(std::make_shared<Level2>(sceneManager));  // Reload the level
            }
            else if (menuButton.getGlobalBounds().contains(static_cast<float>(mousePos.x), static_cast<float>(mousePos.y))) {
                // Go back to the menu using setScene
                sceneManager.setScene(std::make_shared<TitleScreen>(sceneManager));  // Transition to the menu scene
            }
        }
    }
}
